...
class TestParseLine:
    ...
    def test_parse_valid_line_with_dots(self):
        ...
        line = '"Электроэнергия" 2024.11.20 150.75 good'
    ...
    def test_parse_line_with_multiple_spaces(self):
        ...
        line = '"Газ"   2024.01.15   1234.56    excellent'
        result = parse_line_shlex(line)
    ...
    def test_parse_line_with_quoted_resource_having_spaces(self):
        ...
        line = '"Природный газ" 2024.12.01 95.5 normal'
        result = parse_line_shlex(line)
    ...
    def test_parse_line_raises_error_on_invalid_date(self):
        ...
        with pytest.raises(ValueError):
            parse_line_shlex('"Тест" 2024.13.45 100.0 good')


class TestLoadFromFile:
    """Тесты для функции file_open."""

    def test_load_from_valid_file(self):
        """Тест загрузки из корректного файла."""
        with tempfile.NamedTemporaryFile(mode='w', encoding='utf-8', delete=False) as f:
            f.write('"Электроэнергия" 2024.11.20 150.75 good\n')
            f.write('"Водоснабжение" 2024.12.01 95.0 normal\n')
            f.write('"Газ" 2025.01.15 1234.56 excellent\n')
            temp_file = f.name
        ...
        try:
            ...
            assert data[2].quality == "excellent"
        finally:
            os.unlink(temp_file)
    ...
    def test_load_from_file_with_empty_lines(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', encoding='utf-8', delete=False) as f:
            f.write('"Тест1" 2024.01.01 10.0 good\n')
            f.write('\n')
            f.write('"Тест2" 2024.01.02 20.0 normal\n')
            temp_file = f.name
